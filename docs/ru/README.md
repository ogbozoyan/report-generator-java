...
Библиотека предназначена для использования как service-layer (без REST): вы передаёте байты шаблона,
данные токенов и опции генерации, на выходе получаете готовый файл и список предупреждений.
...
- [Архитектура и внутреннее устройство](architecture.md)

Локальные ручные интеграционные сценарии находятся в
